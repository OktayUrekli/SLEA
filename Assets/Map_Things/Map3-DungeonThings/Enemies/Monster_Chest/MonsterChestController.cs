...
public class MonsterChestController : MonoBehaviour
{
    ...
    [SerializeField] float npcSpeedMultiplier;
    ...
    {
        npcAnimator.SetFloat("Speed", npcSpeedMultiplier);
        //npcRb.AddForce(transform.forward * npcSpeedMultiplier * Time.deltaTime,ForceMode.Force);
    ...
    void Patroll()
    {
        if (npcNavAgent.remainingDistance <= npcNavAgent.stoppingDistance) //eðer hedef noktaya ulaþmýþ ise
        {
            ...
            {
                Debug.Log("yeni hedefe doðru yola çýkýldý");
